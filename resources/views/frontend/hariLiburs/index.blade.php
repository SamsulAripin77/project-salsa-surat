        ...
        <div class="col-md-12">
            @can('hari_libur_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.hari-liburs.create') }}">
                            ...
                            <tbody>
                                @foreach($hariLiburs as $key => $hariLibur)
                                    <tr data-entry-id="{{ $hariLibur->id }}">
                                        ...
                                        <td>
                                            @can('hari_libur_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.hari-liburs.show', $hariLibur->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('hari_libur_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.hari-liburs.edit', $hariLibur->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('hari_libur_delete')
                                                <form action="{{ route('frontend.hari-liburs.destroy', $hariLibur->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('hari_libur_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.hari-liburs.massDestroy') }}",
