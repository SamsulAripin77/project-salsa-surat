        ...
        <div class="col-md-12">
            @can('presensi_hadir_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.presensi-hadirs.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $presensiHadir->id }}">
                                        ...
                                        <td>
                                            @can('presensi_hadir_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.presensi-hadirs.show', $presensiHadir->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('presensi_hadir_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.presensi-hadirs.edit', $presensiHadir->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('presensi_hadir_delete')
                                                <form action="{{ route('frontend.presensi-hadirs.destroy', $presensiHadir->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('presensi_hadir_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.presensi-hadirs.massDestroy') }}",
