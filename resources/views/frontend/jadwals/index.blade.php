        ...
        <div class="col-md-12">
            @can('jadwal_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.jadwals.create') }}">
                            ...
                            <tbody>
                                @foreach($jadwals as $key => $jadwal)
                                    <tr data-entry-id="{{ $jadwal->id }}">
                                        ...
                                        <td>
                                            @can('jadwal_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.jadwals.show', $jadwal->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('jadwal_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.jadwals.edit', $jadwal->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('jadwal_delete')
                                                <form action="{{ route('frontend.jadwals.destroy', $jadwal->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('jadwal_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.jadwals.massDestroy') }}",
