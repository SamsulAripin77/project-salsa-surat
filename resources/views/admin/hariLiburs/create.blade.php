...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.hari-liburs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="tgl">{{ trans('cruds.hariLibur.fields.tgl') }}</label>
                <input class="form-control date {{ $errors->has('tgl') ? 'is-invalid' : '' }}" type="text" name="tgl" id="tgl" value="{{ old('tgl') }}">
                @if($errors->has('tgl'))
