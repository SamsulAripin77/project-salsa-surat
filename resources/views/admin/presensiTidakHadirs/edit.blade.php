...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.presensiTidakHadir.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.presensi-tidak-hadirs.update", [$presensiTidakHadir->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="cdate">{{ trans('cruds.presensiTidakHadir.fields.cdate') }}</label>
                <input class="form-control date {{ $errors->has('cdate') ? 'is-invalid' : '' }}" type="text" name="cdate" id="cdate" value="{{ old('cdate', $presensiTidakHadir->cdate) }}">
                @if($errors->has('cdate'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_pegawai') ? 'is-invalid' : '' }}" name="id_pegawai_id" id="id_pegawai_id">
                    @foreach($id_pegawais as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_pegawai_id') ? old('id_pegawai_id') : $presensiTidakHadir->id_pegawai->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('keterangan') ? 'is-invalid' : '' }}" name="keterangan_id" id="keterangan_id">
                    @foreach($keterangans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('keterangan_id') ? old('keterangan_id') : $presensiTidakHadir->keterangan->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('keterangan_lanjut') ? 'is-invalid' : '' }}" type="text" name="keterangan_lanjut" id="keterangan_lanjut" value="{{ old('keterangan_lanjut', $presensiTidakHadir->keterangan_lanjut) }}">
