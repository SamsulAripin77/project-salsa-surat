...
@can('presensi_tidak_hadir_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.presensi-tidak-hadirs.create') }}">
                        ...
                        <tr data-entry-id="{{ $presensiTidakHadir->id }}">
                            ...
                            <td>
                                @can('presensi_tidak_hadir_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.presensi-tidak-hadirs.show', $presensiTidakHadir->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('presensi_tidak_hadir_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.presensi-tidak-hadirs.edit', $presensiTidakHadir->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('presensi_tidak_hadir_delete')
                                    <form action="{{ route('admin.presensi-tidak-hadirs.destroy', $presensiTidakHadir->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('presensi_tidak_hadir_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.presensi-tidak-hadirs.massDestroy') }}",
