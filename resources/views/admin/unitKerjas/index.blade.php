...
@can('unit_kerja_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.unit-kerjas.create') }}">
                ...
                <tbody>
                    @foreach($unitKerjas as $key => $unitKerja)
                        <tr data-entry-id="{{ $unitKerja->id }}">
                            ...
                            <td>
                                @can('unit_kerja_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.unit-kerjas.show', $unitKerja->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('unit_kerja_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.unit-kerjas.edit', $unitKerja->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('unit_kerja_delete')
                                    <form action="{{ route('admin.unit-kerjas.destroy', $unitKerja->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('unit_kerja_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.unit-kerjas.massDestroy') }}",
